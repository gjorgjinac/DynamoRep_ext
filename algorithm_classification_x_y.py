...
x_columns=list(filter(lambda c: np.any([f'_dim_{d}' in c for d in range(0,dimension)]), feature_df.columns))
y_columns = list(filter(lambda c: f'_dim_{dimension}' in c, feature_df.columns))
print(x_columns)
print(y_columns)
...
for statistic in ['x','y']:
    for train_seed in seeds:
        ...
        for train_index, test_index in kf.split(problem_ids):
            ...
            if os.path.isfile(f'{result_dir}/{run_name}_test_preds.csv') and False:
