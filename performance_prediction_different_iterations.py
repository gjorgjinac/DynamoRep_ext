...
#nohup python performance_prediction.py algorithm_name, difference, iterations,precision_predict_iteration, predict_log_precision, dimension 
...
precision_predict_iteration = int(arguments[4])
id_columns=['algorithm_name', 'problem_id', 'instance_id', 'seed']
predict_log_precision=True if arguments[5].lower()=='true' else False
seeds = [200,400,600,800,1000]
dimension=int(arguments[6])
...
if os.path.isfile(feature_df_file):
    ...
    precision_df = get_precision(sample_df.query('iteration<=@precision_predict_iteration') ,dimension)
...
for train_seed in seeds:
    ...
    if 'config' not in argument_algorithm_names:
        global_name=f'dim_{dimension}_{"_".join(algorithm_names)}_it_{iteration_min}-{iteration_max}_predict_it_{precision_predict_iteration}_instance_count_{instance_count}_seed_{train_seed}_log_{predict_log_precision}' + ('_differenced' if difference else '')
    else:
        global_name=f'dim_{dimension}_{argument_algorithm_names}_it_{iteration_min}-{iteration_max}_predict_it_{precision_predict_iteration}_instance_count_{instance_count}_seed_{train_seed}_log_{predict_log_precision}'+ ('_differenced' if difference else '')
